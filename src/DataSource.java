...
public class DataSource extends Activity {
	...
	public enum TYPE {
		WIKIPEDIA, BUZZ, TWITTER, OSM, MIXARE, ARENA , GWNU
	};
	...
	public String createRequestParams(double lat, double lon, double alt, float radius, String locale) {
		...
		if (!ret.startsWith("file://")) {
			...
			case WIKIPEDIA:
				...
				ret += "?lat=" + lat + "&lng=" + lon + "&radius="+ geoNamesRadius + "&maxRows=50" + "&lang=" + locale+ "&username=mixare";
				
				break;

			case BUZZ:
				ret += "&lat=" + lat + "&lon=" + lon + "&radius=" + radius* 1000;
				break;
			...
			case MIXARE:
				ret += "?latitude=" + Double.toString(lat) + "&longitude="+ Double.toString(lon) + "&altitude="+ Double.toString(alt) + "&radius="+ Double.toString(radius);
				break;

			case ARENA:
				ret += "&lat=" + Double.toString(lat) + "&lng="+ Double.toString(lon);
				break;
	...
	public String serialize() {
		return this.getName() + "|" + this.getUrl() + "|" + this.getTypeId() + "|" + this.getDisplayId() + "|" + this.getEnabled();
	}
	...
	public String toString() {
		return "DataSource [name=" + name + ", url=" + url + ", enabled=" + enabled + ", type=" + type + ", display=" + display + "]";
	}
	...
	public boolean isUrlWellFormed() {
		return getUrl() != null || !getUrl().isEmpty()|| "http://".equalsIgnoreCase(getUrl());
	}
}
